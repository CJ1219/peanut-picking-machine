...
class ProductionDatabase:
    ...
    def list_production_batches(self) -> list[sqlite3.Row]:
        with self.connect() as connection:
            return connection.execute(
                ...
                SELECT
                    w.*,
                    COUNT(p.id) AS peanut_count,
                    COALESCE(SUM(CASE WHEN p.classification = 'normal' THEN 1 ELSE 0 END), 0)
                        AS normal_count,
                    COALESCE(SUM(CASE WHEN p.classification IN ('mold', 'small') THEN 1 ELSE 0 END), 0)
                        AS ng_count
                FROM work_orders w
    ...
    def delete_production_batches(self, work_order_ids: list[int]) -> list[str]:
        ids = sorted({int(value) for value in work_order_ids})
        if not ids:
            return []
        placeholders = ",".join("?" for _ in ids)
        with self.connect() as connection:
            image_rows = connection.execute(
                f"SELECT image_path FROM peanuts WHERE work_order_id IN ({placeholders})",
                ids,
            ).fetchall()
            sample_rows = connection.execute(
                f"""
                SELECT image_path, metadata_path, x_labels_path
                FROM training_samples WHERE work_order_id IN ({placeholders})
                """,
                ids,
            ).fetchall()
            file_paths = [str(row["image_path"]) for row in image_rows]
            for row in sample_rows:
                file_paths.extend(
                    str(row[key]) for key in ("image_path", "metadata_path", "x_labels_path")
                    if row[key]
                )
            connection.execute(
                f"DELETE FROM training_samples WHERE work_order_id IN ({placeholders})", ids
            )
            connection.execute(
                f"DELETE FROM peanuts WHERE work_order_id IN ({placeholders})", ids
            )
            connection.execute(
                f"DELETE FROM anomalies WHERE work_order_id IN ({placeholders})", ids
            )
            connection.execute(
                f"DELETE FROM work_orders WHERE id IN ({placeholders})", ids
            )
        return file_paths
        ...
        with self.connect() as connection:
            cursor = connection.execute(
                ...
                INSERT INTO peanuts (
                    ...
                    lane, boundary_between_json, eject_lanes_json,
    ...
    def set_training_sample_x_labels(
        self, sample_id: int, x_labels_path: Path | None, reason: str
    ) -> None:
        with self.connect() as connection:
            connection.execute(
                """
                UPDATE training_samples
                SET x_labels_path = ?, review_reason = ?, updated_at = ?
                WHERE id = ?
                """,
                (
                    str(x_labels_path) if x_labels_path else None,
    ...
    def set_training_sample_review_status(self, sample_id: int, status: str) -> None:
        with self.connect() as connection:
            connection.execute(
                "UPDATE training_samples SET review_status = ?, updated_at = ? WHERE id = ?",
                (status, datetime.now().isoformat(timespec="seconds"), sample_id),
            )
